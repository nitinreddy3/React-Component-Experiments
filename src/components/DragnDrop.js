...
import exampleTrees from "../data/example-data";
...
class DragnDrop extends Component {
  ...
  onDragEnd = (sourcePosition, destinationPosition) => {
    ...
    } = this.removeItemFromTree(sourceTree, sourcePosition);

    const movingItem = sourceTree.items[removedItemId];
    const childItems = this.getAllItemChildren(sourceTree, movingItem.id);
    ...
    const newDestTree = this.addItemToTree(
  ...
  getAllItemChildren(tree, itemId) {
    ...
    childIds.forEach(childId =>
      children.push(
        tree.items[childId],
        ...this.getAllItemChildren(tree, childId)
      )
    );
    return children;
  }
  ...
  // from @atlaskit/tree/utils/tree (not exported)
  addItemToTree = (tree, position, item) => {
